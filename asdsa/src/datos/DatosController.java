...
public class DatosController {
    ...
    void initialize() {
    		...
    		if(table1 !=null) {
    			...
    			table1.getSelectionModel().getSelectedItems().addListener(new InvalidationListener() {
					...
					public void invalidated(Observable arg0) {
						...
						texto.setText("Empleado:" +table1.getSelectionModel().getSelectedItem().getFirstName() + table1.getSelectionModel().getSelectedItem().getLastName());
						}
						else {
							cambio.setText("El empleado ha sido borrado con exito");
						}
					}
				});
    	    	...
    	    	texto.addEventHandler(MouseEvent.MOUSE_PRESSED, e->{
    				texto.setCursor(Cursor.CLOSED_HAND);
    			});
    	    	texto.addEventFilter(MouseEvent.MOUSE_RELEASED, e->{
    	    		texto.setCursor(Cursor.OPEN_HAND);
    	    	});
    	    	texto.addEventFilter(MouseEvent.MOUSE_ENTERED, e->{
    	    		texto.setCursor(Cursor.OPEN_HAND);
    	    	});
			...
			texto.setOnDragDetected((event) -> {
				// Se indica que el modo de transferencia será del tipo movimiento
				Dragboard db = texto.startDragAndDrop(TransferMode.MOVE);
				// Se transfiere el String del texto inicial
				ClipboardContent content = new ClipboardContent();
 				content.putString(texto.getText());
